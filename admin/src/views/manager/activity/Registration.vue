<template>
    <div>
      <div class="search">
        <el-form :data="activityRegistration" class="form-search">
          <el-form-item label="活动名称："><el-input placeholder="请输入活动名称查询" style="width: 200px"
              v-model="activityRegistration.name"></el-input></el-form-item>
          <el-form-item label="学生姓名："><el-input placeholder="请输入学号查询" style="width: 200px"
              v-model="activityRegistration.studentId"></el-input></el-form-item>
      ...
      <div class="table">
        <el-table :data="activityRegistrations" strip @selection-change="handleSelectionChange">
          ...
          <el-table-column prop="registrationCount" label="报名人数"></el-table-column>
      ...
      <el-dialog title="修改或添加信息" :visible.sync="dialogFormVisible">
        <el-form :model="form" label-width="100px">
          <el-form-item label="活动名称" prop="course">
          ...
          <el-form-item label="请假人数" prop="registrationCount">
            <el-input v-model="form.registrationCount"></el-input>
          </el-form-item>
          <el-form-item label="回复" prop="reviewResponse">
  ...
  export default {
    ...
    methods: {
      fetchFormData() {
        getAll("activityRegistration").then(res => {
          this.activityRegistrations = res.data
          console.log(this.activityRegistrations)
        })
      ...
      save() {
        if (!(this.form.id === 0)) {
          updateOne(this.form, 'activityRegistration').then(res => {
      ...
      del(id) {
        ...
        this.$confirm('您确定删除吗？', '确认删除', { type: "warning" }).then(response => {
          console.log(id)
          delOne(id, 'activityRegistration').then(res => {
      ...
      delBatch() {   // 批量删除
        ...
        this.$confirm('您确定批量删除这些数据吗？', '确认删除', { type: "warning" }).then(response => {
          ...
          this.$request.delete('/activityRegistration/deleteBatch', { data: this.ids }).then(res => {
            if (res.code === 20000) {   // 表示操作成功
              this.$message.success('操作成功')
              this.fetchFormData();
            } else {
      ...
      reset() {
        this.activityRegistration.name = null
        this.activityRegistration.activityRegistrationTeacherName = null
        this.fetchFormData()
      },
      find() {
        console.log(this.activityRegistration)
        queryOne(this.activityRegistration, 'activityRegistration').then(res => {
          this.activityRegistrations = res.data;
        })
      },
