<template>
  <div>
      <div class="search">
          <el-form :data="activity" class="form-search">
              <el-form-item label="活动名称："><el-input placeholder="请输入活动名称查询" style="width: 200px"
                      v-model="activity.name"></el-input></el-form-item>
              <el-form-item label="活动地点："><el-input placeholder="请输入活动地点查询" style="width: 200px"
                      v-model="activity.location"></el-input></el-form-item>
      ...
      <div class="table">
          <el-table :data="activitys" strip @selection-change="handleSelectionChange">
...
export default {
  ...
  methods: {
      ...
      save() {
          if (!(this.form.id === 0)) {
              updateOne(this.form, 'activity').then(res => {
              ...
              addOne(this.form, 'activity').then(res => {
                  if (res.code === 20000) {
      ...
      delBatch() {   // 批量删除
          ...
          this.$confirm('您确定批量删除这些数据吗？', '确认删除', { type: "warning" }).then(response => {
              ...
              this.$request.delete('/activity/deleteBatch', { data: this.ids }).then(res => {
                  if (res.code === 20000) {   // 表示操作成功
                      this.$message.success('操作成功')
                      this.fetchFormData();
                  } else {
