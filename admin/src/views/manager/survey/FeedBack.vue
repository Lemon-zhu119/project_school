<template>
  <div>
    <div class="search">
      <el-form :data="surveyFeedback" class="form-search">
        <el-form-item label="问卷名称："><el-input placeholder="请输入问卷名称查询" style="width: 200px"
            v-model="surveyFeedback.name"></el-input></el-form-item>
        <el-form-item label="当前状态："> 
          <el-select v-model="surveyFeedback.status" placeholder="请选择状态">
    ...
    <div class="table">
      <el-table :data="surveyFeedbacks" strip @selection-change="handleSelectionChange">
        <el-table-column type="selection" width="55" align="center"></el-table-column>
        <el-table-column prop="id" label="序号" width="70" align="center" sortable></el-table-column>
        <el-table-column prop="name" label="问卷名称"></el-table-column>
        <el-table-column prop="time" label="反馈时间">
        </el-table-column>
        <el-table-column prop="parentUsername" label="反馈家长"></el-table-column>
        <el-table-column prop="studentId" label="反馈学号"></el-table-column>
        <el-table-column  :show-overflow-tooltip="true" prop="feedbackContent" label="反馈内容"></el-table-column>
    ...
    <el-dialog title="修改或添加信息" :visible.sync="dialogFormVisible">
      <el-form :model="form" label-width="100px">
        ...
        <el-form-item label="反馈内容" prop="feedbackContent" >
          <el-input  :disabled="true" type="textarea" placeholder="请输入内容" v-model="form.feedbackContent"></el-input>
        </el-form-item>
        <!-- ...其他表单项... -->
      </el-form>
...
export default {
  ...
  methods: {
    fetchFormData() {
      getAll("surveyFeedback").then(res => {
        this.surveyFeedbacks = res.data
        console.log(this.surveyFeedbacks)
      })
    ...
    save() {
      if (!(this.form.id === 0)) {
        updateOne(this.form, 'surveyFeedback').then(res => {
    ...
    del(id) {
      ...
      this.$confirm('您确定删除吗？', '确认删除', { type: "warning" }).then(response => {
        console.log(id)
        delOne(id, 'surveyFeedback').then(res => {
    ...
    delBatch() {   // 批量删除
      ...
      this.$confirm('您确定批量删除这些数据吗？', '确认删除', { type: "warning" }).then(response => {
        ...
        this.$request.delete('/surveyFeedback/deleteBatch', { data: this.ids }).then(res => {
          if (res.code === 20000) {   // 表示操作成功
            this.$message.success('操作成功')
            this.fetchFormData();
          } else {
    ...
    reset() {
      this.surveyFeedback.name = null
      this.surveyFeedback.status = null
      this.fetchFormData()
    },
    find() {
